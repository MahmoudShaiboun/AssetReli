"""
Test electrical_fluting on live MQTT system
"""
import json
...
def publish_anomaly():
    """Publish electrical_fluting anomaly pattern to MQTT"""
    
    print("="*80)
    print("  TESTING ELECTRICAL FLUTING - LIVE MQTT SYSTEM")
    print("="*80)
    ...
    print(f"✅ Expected prediction: electrical_fluting")
    ...
    try:
        ...
        for i, reading in enumerate(window_data, 1):
            ...
            reading["sensor_id"] = "electrical_fluting_test"
        ...
        print("  CHECK RESULTS")
        print("="*80)
        print("\n1. Check mqtt-ingestion logs:")
        print("   docker logs aastreli-mqtt-ingestion --tail 50")
        print("\n2. Check MongoDB for prediction:")
        print('   docker exec -it aastreli-mongodb mongosh assetreli --eval "db.sensor_readings.find({sensor_id: \'electrical_fluting_test\'}).sort({timestamp: -1}).limit(1).pretty()"')
        print("\n3. Expected prediction: electrical_fluting")
        print("\n" + "="*80)
