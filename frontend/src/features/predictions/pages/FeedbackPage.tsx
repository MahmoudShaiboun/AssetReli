...
export default function FeedbackPage() {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      setSelectedIds(new Set(readings.map(r => r._id)));
    } else {
  ...
  const handleSubmitFeedback = async () => {
    try {
      ...
      const feedbackPromises = selectedReadings.map(async (reading) => {
        ...
        const response = await apiClient.post('/feedback', features, {
          params: {
  ...
  const formatValue = (value: number | undefined | null) => value != null ? value.toFixed(2) : 'N/A';

  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h4" gutterBottom>Sensor Data Feedback & Anomalies</Typography>
      ...
      <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <Chip label={`${selectedIds.size} selected`} color={selectedIds.size > 0 ? "primary" : "default"} />
          <Chip label={`${readings.filter(r => r.prediction && r.prediction.toLowerCase() !== 'normal').length} anomalies`} color="warning" />
          <Chip label={`${readings.filter(r => r.has_feedback === true).length} with feedback`} color="success" />
        </Box>
        <Button variant="contained" color="primary" onClick={handleOpenDialog} disabled={selectedIds.size === 0}>
                  ...
                  return (
                    <TableRow
                      ...
                      sx={{
                        ...
                        '&:hover': { bgcolor: isAnomaly ? 'warning.main' : 'action.hover' }
                      }}
                    >
                      <TableCell padding="checkbox">
                        <Checkbox
                          checked={selectedIds.has(reading._id)}
                          onChange={(e) => { e.stopPropagation(); handleSelectOne(reading._id); }}
                        />
                      </TableCell>
                      <TableCell>
                        {reading.sensor_id}
                        {isAnomaly && <Chip label="ANOMALY" color="error" size="small" sx={{ ml: 1 }} />}
                      </TableCell>
                      ...
                      <TableCell>
                        {reading.prediction ? (
                          <Chip label={reading.prediction} size="small" color={reading.prediction.toLowerCase() === 'normal' ? 'success' : 'error'} />
                        ) : (
                      ...
                      <TableCell>
                        {reading.confidence != null && !isNaN(reading.confidence) ? (
                          <Chip label={`${(reading.confidence * 100).toFixed(0)}%`} size="small" color={reading.confidence > 0.7 ? 'success' : 'warning'} />
                        ) : 'N/A'}
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ pt: 2 }}>
            <TextField select fullWidth label="Correct Label" value={feedbackLabel} onChange={(e) => setFeedbackLabel(e.target.value)} sx={{ mb: 3 }}>
              {feedbackTypes.map((option) => (
                <MenuItem key={option.value} value={option.value}>{option.label}</MenuItem>
              ))}
            </TextField>
            <TextField fullWidth multiline rows={4} label="Notes (Optional)" placeholder="Add any additional information about this feedback..." value={notes} onChange={(e) => setNotes(e.target.value)} />
          </Box>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseDialog} disabled={submitting}>Cancel</Button>
          <Button onClick={handleSubmitFeedback} variant="contained" color="primary" disabled={submitting}>
